...
- [Common services and protocols](#common-services-and-their-protocol-)
...
![remotemac1](images/remotemac1.png)
...
![remotemac2](images/remotemac2.png)
...
![remotemac3](images/remotemac3.png)
...
- both tcp and udp use different ports and both have 65535 ports separately
...
> computers randomly use any port between 49152-65535 as source ports
...
- source and destination ports are placed in the segments , the segment is then encapsulated by ip packets (layer 3)
- combination of source ip and source port or destination ip and destination port are called socket pairs.
- check the image it shows a pc requesting both web services and ftp connection from a web server

![socketpairs](images/socketpairs.png)

- in the above exampe we can see layer 2 mac addressed and layer 3 ip addresses
- it also includes source port 1305 (randomly generated by host) and destination port 21 (default for FTP services)
- another request also has same layer 2,3 addresses but source port is 1099 and destination port is 80 (default for web services)
- socket is used to find the server and service requested by the client 
- client socket looks like `192.168.1.5:1099` and server socket `192.168.1.7:80`
- together we call these sockets ,socket pairs :192.168.1.5:1099, 192.168.1.7:80
- sockets enable clients and servers to distinguish between multiple processes running together simultaneously
- source port number also acts as a return address ,the transport layer keeps track of the port and the application and when response is returned it can be forwarded to correct application

## Netstat Cmd :

- netstat command is used to check active tcp and udp connections currently running on our pc
- cmd `netstat`
...
- uri is a identifier for websites it maybe in form of url or urn (uniform resource name) 
- web services , web resources and api's are also uri like `https://api.example.com/v1/users/123`

1. **Uniform Resource Name (URN) :** urn is a type of uri it identifies the resource by its name and not location it is like `urn:isbn:0451524934`

> we dont really use urns only few websites registered with iana have them and they never expire even if the domain is changed etc 

2. **Uniform Resource Locator (URL) :** url is the network location of a specific resource on the network , http or https urls are typically used with web browsers other protocols like ftp ,sftp and ssh also can be used as urls

**Url Structure :**

    protocol://domain:port/path?query

1. protocol is http,https,ftp,mail
2. domain is the domain name like google.com
3. port is 80 for web servers 
4. path is like a page or any resource on web browser like /home
5. query is for asking a specific type of data using key-value pairs like in google search ?q=abc (here q is a key and abc is a value which is searched by a user) we may have ?sort=price , category=shoes or both together ?category=shoes&sort=price etc

    https://google.com:80/search?q=abc

---

# Common services and their protocol :

| Protocol | Description |
| --- | --- |
| **Domain Name System (DNS)** | gives ip addresses of domains |
| **Secure Shell (SSH)** | for accesssing devices remotely |
| **Simple Mail Transfer Protocol (SMTP)** | for sending email messages and attachments from client to servers and then from servers to other servers. |
| **Post Office Protocol (POP)** | Used by email clients to retrieve email and attachments from a remote server and emails are deleted after seen once. |
| **Internet Message Access Protocol (IMAP)** | Used by email clients to retrieve and email and attachments on a remote server and emails are saved until deleted. |
| **Dynamic Host Configuration Protocol (DHCP)** | Used to automatically configure devices with IP address and other necessary information  |
| **Hypertext Transfer Protocol (HTTP)** | Used by web browsers to request web pages and web servers to transfer the files that make up web pages  |
| **File Transfer Protocol (FTP)** | Used for file transfer between systems. |

## Domain Name System (DNS):

- domain name system is used to get ip address of a domain name
- as we know websites are hosted on ip addresses and we cant access a website by a domain name
- so when we use web browser and put a website name we use dns protocol to get the ip of the website first
- we cant save all domains and their ips locally cuz there are millions of website so we have dns servers

> we can use nslookup in terminal to see ip addresses of domains just type nslookup google.com and u will get its ip ,you can put that ip in the browser and u will get the website

## HTTP And HTML :

- when we use web browser and search a domain we get ip of the domain from dns but we get webpage content from http procol on port 80 which gives us the html webpage
- the html is a markup language which tells the browser how to format web page and what graphics or fonts to use
- http is not secure so now we use https which have added security and it uses port 443

## File Transfer Protocol (FTP) :

-  ftp is used to transfer files between devices by a host running a ftp server and a ftp client accessing the files on the server 
-  ftp server uses different ports to communicate it uses port 21 to begin th ftp session then it uses port 20 to transfer files
-  it enables clients to manage files remotely by cmds like delete and rename etc
-  ftp is old and not secure and its data is not encrypted and now we use sftp and ftps
  

## Telnet And SSH :

- telnet was made in 1970s when there were only text based computers with only terminals and no gui 
- with telnet ppl could remotely manage files etc of a device without any physical connection
- telnet uses port 23 for connection 

![telnet](images/telnet.png)

- telnet is not secure enough as it also doesnt encrypt the data and sends in plain text 
- secure shell (SSH) protocol is a secure method and alternate of telnet stronger authorization and encryption

![sshtelnet](images/sshtelnet.png)

## Emails :

- emails are mostly used in online communication with the help of email servers and clients.
- email servers save and configure mails and users need to use email clients specific to their email servers to access the mails which are mostly web based ex : gmail , hotmail,yahoo
- mailboxes are identified by the format user@company.domain
- protocols used in processing emails are SMTP , POP3 , IMAP4

1. Simplel Mail Transfer Protocol (SMTP) : Smtp is used for sending mails from the user to the mail server and from one mail server to another mail server . it uses port 25
2. Post Office Protocol (POP3) : Pop clients are users whose mails are stored on the server until they read it , when they read the mail the mails are deleted automatically . it uses port 110
3. Internet Message Access Protocol (IMAP4) : Imap clients users mails are saved on the server until they delete it themselves . it uses port 143

## Internet Phone Calls : 

- we can make phone calls using internet by voice over ip (VoIP) technology
- it changes voice signals into digital data and voice data is encapsulated into ip packets which carry phone call through the network
- in internet phone call software we need to select a username to show to the received users
- when calling another cell phone or landline public switched telephone network (PSTN) gateway is required
...
you  can use some commands to identify network problems like

## ipconfig :

- ipconfig commands shows the user its ip address subnet mask and default gateway 
```
ipconfig 
```
- `ipconfig /all` shows more information like mac address , ip address of default gateway, dns servers , dhcp server information

> you can use it to see if the host has correct ip configuration and if host knows the location of dns servers

- `ipconfig /release` it is used to release the ip address assigned to you by the dhcp and then u can use `ipconfig/renew` to get a new ip address

## ping command :

- ping command is used to check if we can reach to another device through ip
- we can either ping by ip or name of destination host like
  ```
  ping google.com
  ping 192.168.12.23
  ```
- when ping is sent to an ip address an echo request packet is sent across the network to the ip  
- if destination host receives it ,it will reply back with echo reply to the sender and when sender receives it the connection is verified
- if ping is sent to a name then first it will get its ip by dns and continue its process
- if ping to an ip works but ping to its name doesn't then there is a problem with the dns
- if ping to default gateway fails its likely a issue in local network or router 
- ping may fail due to firewall in sending or receiving device or a router between them

## nslookup command :

- we can use nslookup to directly check if the dns is working 
- just type nslookup with a host name
```
nslookup google.com
```
...
- tracert command is used to display the route a packet takes from your computer to the destination
- you can see the ips of routers that are between your computer and the destination 
- some routers may hide their ip or just ignore tracert command 
- you can use it to troubleshoot where the connection went wrong 

```
 tracert google.com 
 ```

> like tracert to check routers between destination we also have a command in linux to know the dns servers used to get the hosts ip the commands is `dig +trace google.com`
